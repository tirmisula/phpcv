...
class mysql {
 ...
 if ($delete)
 {
    $sets = explode('@',$result[0]['sets']);
    $new_sets = "";
    foreach ($sets as $v){
        if((int)substr($v,0,1)==$set_id){
            $delete_content = $v;
            continue;
        }
        if ((int)substr($v,0,1)>$set_id){
            $v = substr_replace($v,strval((int)substr($v,0,1)-1),0,1);
        }
        $new_sets = $new_sets . $v . '@';
    }
    //删除最后一个@
    $new_sets = rtrim($new_sets,'@');
 }
