...
class mysql {
 ...
 //select    查
 $db->select("heartstone_user", "password", "name = '$user_name'");
 $result = $db->fetchArray(MYSQL_ASSOC);
 //print_r($result);
 if(strcmp($result[0]['password'],$user_password)==0){ 
     echo "登录成功/";
     $sql = "SELECT `nickname`, `expack`, `gold`, `id`, `cash`, `cards` FROM `heartstone_user` WHERE `name` = " . "'" . $user_name . "'";
     $db->query($sql);
     $result = $db->fetchArray(MYSQL_ASSOC);
     //print_r($result);
     echo $result[0]['nickname'] . "/" . $result[0]['expack'] . "/" . $result[0]['gold'] . "/" . $result[0]['id'] . "/" . $result[0]['cash'] . "/" . $result[0]['cards'];
